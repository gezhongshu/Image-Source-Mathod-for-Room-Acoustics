...
all = 1; % decide if display the single paths
refl_order = 3; % reflection order 1==first only,2==second only,3==all,-1==none
% select a specific path in 1st or 2nd reflections, single number represents 
% 1st order reflection, two number vector represents 2nd order reflection
% if refl_order = 3, this handle has no function
path = [9 10]; 
...
if refl_order == 2 || refl_order == 3
    for g = 1:1:wnum
        for h = 1:1:wnum
            ...
            amplitude{1,2}{g,h} = temp*beta(g)*beta(h);
...
%                 disp([n (time{1,1}{1,n}-dt)*1000 max(amplitude{1,1}{1,n})]);
                disp([n time{1,1}{1,n} max(amplitude{1,1}{1,n})]);
            end
        end
    else
        for n = path
...
%                         disp([h g (time{1,2}{g,h}-dt)*1000 max(amplitude{1,2}{g,h})]);
                        disp([h g time{1,2}{g,h} max(amplitude{1,1}{1,n})]);
                    end
                end
            end
...
%             t(t==max(t))= 1;
            t_start = find(t==max(t));
            IR(t_start:t_start+size(amplitude{1,2}{path(2),path(1)},2)-1) = amplitude{1,2}{path(2),path(1)};
            disp('Second Reflection');
